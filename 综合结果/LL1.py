    ...
    while flag:
        if count == 0:
            ...
            if x in Vt:
                ...
                print('%d\t\t%s\t\t%s' % (count, printstack(stack), printstr(str, location)))
                file.write('%d\t\t%s\t\t%s' % (count, printstack(stack), printstr(str, location))+'\n')

            else:
                # x是非终结符  s是产生式的右部
                print('%d\t\t%s\t\t%s\t\t%s->%s' % (count, printstack(stack), printstr(str, location), x, s))
                file.write('%d\t\t%s\t\t%s\t\t%s->%s' % (count, printstack(stack), printstr(str, location), x, s)+'\n')
                sttt = x + '=>' + s
